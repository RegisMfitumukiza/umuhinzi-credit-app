...
import {
  ...
  updateCropService,
  deleteCropService,
  getCropPerformanceHistoryService,
...
export const getCropPerformanceHistory = asyncHandler(
  async (req: Request, res: Response) => {
    if (!req.user) throw new APIError("User not authenticated", 401);
    ...
    const result = await getCropPerformanceHistoryService(req.user.id, cropName, {
      skip,
      limit,
    });
    ...
    res.status(200).json({
      success: true,
      message: "Crop performance history fetched successfully",
      data: result,
    });
  }
);
