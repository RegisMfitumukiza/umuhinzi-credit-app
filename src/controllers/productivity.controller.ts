...
export const verifyYieldRecord = asyncHandler(
  async (req: Request, res: Response) => {
    if (!req.user) throw new APIError("User not authenticated", 401);

    const record = await verifyYieldRecordService(
      String(req.params.id),
      req.user.id,
      req.user.role,
      req.body,
    ...
    logger.info("Yield record verification updated", {
      recordId: req.params.id,
      userId: req.user.id,
    ...
    res.status(200).json({
      success: true,
      message: "Yield record verification updated successfully",
      data: record,
    });
  }
);
