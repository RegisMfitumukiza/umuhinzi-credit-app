...
export const verifyFarmLocation = asyncHandler(
  async (req: Request, res: Response) => {
    if (!req.user) throw new APIError("User not authenticated", 401);

    const farm = await verifyFarmLocationService(
      String(req.params.id),
      req.user.id,
      req.user.role,
      req.body,
    ...
    logger.info("Farm location verification updated", {
      farmId: req.params.id,
      updatedBy: req.user.id,
    ...
    res.status(200).json({
      success: true,
      message: "Farm location verification updated successfully",
      data: farm,
    });
  }
);
