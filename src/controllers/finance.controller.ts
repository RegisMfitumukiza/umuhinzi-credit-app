...
import {
  ...
  recalculateFinancialSummaryService,
...
export const recalculateFinancialSummary = asyncHandler(
  async (req: Request, res: Response) => {
    if (!req.user) throw new APIError("User not authenticated", 401);

    const summary = await recalculateFinancialSummaryService(
      String(req.params.id),
      req.user.id,
      getRequestContext(req)
    );

    logger.info("Financial summary recalculated", {
      summaryId: req.params.id,
      userId: req.user.id,
    });
