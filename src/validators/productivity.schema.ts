...
export const yieldVerificationStatusSchema = z.enum([
  "VERIFIED",
  "REJECTED",
]);

export const yieldVerificationMethodSchema = z.enum([
  "FIELD_VISIT",
  "COOPERATIVE_COLLECTION_RECORD",
]);
...
export const verifyYieldRecordSchema = z.object({
  params: z.object({ id: uuidSchema("Invalid yield record ID") }),
  body: z
    .object({
      status: yieldVerificationStatusSchema,
      method: yieldVerificationMethodSchema.optional(),
      evidenceReference: z.string().trim().min(2).max(100).optional(),
      verifiedActualYield: z.number().positive().optional(),
      verifiedUnit: z.string().trim().min(1).max(20).optional(),
      verificationNote: z.string().trim().min(5).max(700).optional(),
    })
    .refine((data) => data.status !== "VERIFIED" || !!data.method, {
      message: "Verification method is required when verifying a yield record",
      path: ["method"],
    })
    .refine(
      (data) =>
        data.status !== "VERIFIED" ||
        data.method !== "COOPERATIVE_COLLECTION_RECORD" ||
        !!data.evidenceReference,
      {
        message: "Collection record reference is required for cooperative collection verification",
    ...
    .refine(
      (data) =>
        data.status !== "VERIFIED" ||
        data.method !== "FIELD_VISIT" ||
        !!data.verificationNote,
      {
        message: "Field visit note is required for field visit verification",
        path: ["verificationNote"],
      }
    )
    .refine((data) => data.status !== "REJECTED" || !!data.verificationNote, {
      message: "Rejection note is required when rejecting a yield record",
      path: ["verificationNote"],
    }),
});
...
registry.register("VerifyYieldRecordInput", verifyYieldRecordSchema);
...
export type VerifyYieldRecordInput = z.infer<typeof verifyYieldRecordSchema>["body"];
