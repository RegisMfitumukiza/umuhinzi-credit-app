...
/**
 * @swagger
 * /api/v1/farms/{id}/location-verification:
 *   patch:
 *     summary: Verify farm location
 *     description: ADMIN can verify any farm. COOPERATIVE_MANAGER can verify farms belonging to active members of their cooperative.
 *     tags: [Farms]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *     responses:
 *       200:
 *         description: Farm location verification updated successfully
 */
router.patch(
  "/:id/location-verification",
  authenticate,
  authorizeRoles("ADMIN", "COOPERATIVE_MANAGER"),
  validate(verifyFarmLocationSchema),
  verifyFarmLocation
);
