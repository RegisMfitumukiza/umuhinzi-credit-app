...
function scoreLocationVerification(
  farms: Array<{
  ...
  if (farms.length === 0) {
    return { score: 0, description: "No farm location data available" };
  }

  const verifiedWithCoordinates = farms.filter(
    (farm) =>
      farm.locationVerificationStatus === "VERIFIED" &&
      farm.latitude != null &&
      farm.longitude != null
  ).length;

  const verifiedAddressOnly = farms.filter(
    (farm) =>
      farm.locationVerificationStatus === "VERIFIED" &&
      (farm.latitude == null || farm.longitude == null)
  ).length;

  const pendingWithCoordinates = farms.filter(
    (farm) =>
      farm.locationVerificationStatus === "PENDING" &&
      farm.latitude != null &&
      farm.longitude != null
  ).length;

  const coordinateRecords = farms.filter(
    (farm) => farm.latitude != null && farm.longitude != null
  ).length;

  if (verifiedWithCoordinates > 0) {
    return {
      score: 100,
      description: `${verifiedWithCoordinates} verified farm location(s) with GPS coordinates`,
    };
  }

  if (verifiedAddressOnly > 0) {
    return {
      score: 75,
      description: `${verifiedAddressOnly} verified farm location(s) using address or local confirmation`,
    };
  }

  if (pendingWithCoordinates > 0) {
    return {
      score: 45,
      description: `${pendingWithCoordinates} farm location(s) with GPS coordinates pending verification`,
    };
  }
  ...
  if (coordinateRecords > 0) {
    return {
      score: 35,
      description: `${coordinateRecords} unverified farm location(s) with GPS coordinates`,
    };
  }

  return {
    score: 20,
    description: "Farm address recorded without verified GPS coordinates",
  };
}
...
export const generateCreditScoreService = async (
  ...
  const [
    ...
    prisma.yieldRecord
      ...
      .then((records) =>
        records.map((record) => ({
          actualYield: record.verifiedActualYield ?? record.actualYield,
  ...
  > = {
    ...
    LOCATION_VERIFICATION: scoreLocationVerification(farms),
  };
  ...
  const creditScore = await prisma.$transaction(async (tx) => {
    const score = await tx.creditScore.create({
      data: {
        ...
        locationVerificationScore: factorResults.LOCATION_VERIFICATION.score,
        summary,
        factors: {
          create: (
