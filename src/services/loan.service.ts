...
const resolveInstitutionIdFromUser = async (userId: string) => {
  ...
  if (institution.status !== "ACTIVE") {
    throw new APIError("Institution must be active before disbursing loans.", 403);
  }
...
export const disburseLoanService = async (
  ...
  if (loan.institutionId && loan.institution?.status !== "ACTIVE") {
    throw new APIError("Loans can only be disbursed by an active institution.", 400);
  }
